 ...
 * 
 * This file provides functions for managing data storage in the EEPROM directory.
 * Functions handle reading, writing, and deleting data blocks associated with block IDs.
 * 
 */
 ...
 * 
 * This function reads the key-map table from EEPROM and populates the directory's
 * key_map structure in memory.
 * 
 ...
 * 
 * This function reads data from multiple blocks in EEPROM and allocates a buffer
 * to store the result. The caller is responsible for freeing the output buffer.
 * 
 * @param directory Pointer to the initialized directory structure.
 ...
 * 
 * This function writes data to multiple blocks in EEPROM. Each block is BLOCK_SIZE bytes.
 * If the data size is smaller than the total block size, remaining bytes are padded with zeros.
 * 
 * @param directory Pointer to the initialized directory structure.
 * @param ids Array of block IDs (up to 4, terminated by BLOCK_COUNT).
 ...
 * 
 * This function frees the blocks associated with the given IDs, making them available
 * for reuse. The blocks are marked as free in the allocation table.
 * 
 * @param directory Pointer to the initialized directory structure.
